...
'''try:
    ...
    if dna not in 'A':
        raise ValueError('Por favor, insira apenas bases A, G, C e T.')
    elif dna not in 'G':
        raise ValueError('Por favor, insira apenas bases A, G, C e T.')
    elif dna not in 'C':
        raise ValueError('Por favor, insira apenas bases A, G, C e T.')
    ...
    dna = dna.replace('A', 'U').replace('T', 'A')
    rna = []

    for i in dna:
        rna.append(i)

    for i, j in enumerate(rna):
        if j == 'G':
            rna[i] = 'C'
        elif j == 'C':
            rna[i] = 'G'
...
try:
    ...
    print("A senha é: {}".format(dados['mes'] + '$' + dados['dia'][::-1] + '#' + dados['dia'] + '!' + dados['mes'][::-1] + '\\' + dados['ano']))
